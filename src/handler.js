...
const getAllBooksHandler = (request, h) => {
  ...
  if (name !== undefined) {
    const filtered = allBooks.filter((n) => n.name.toLowerCase().includes(name.toLowerCase()));
    ...
    const filtered = allBooks.filter((n) => n.reading === read);
    console.log(reading);
    if (filtered !== undefined) {
      const books = filtered.map((book) => ({
        id: book.id,
    ...
    const filtered = allBooks.filter((n) => n.finished === finish);
    if (filtered !== undefined) {
      const books = filtered.map((book) => ({
        id: book.id,
        name: book.name,
